class Camp < ApplicationRecord
  ...
  scope :full, -> { joins('join "registrations" on "camps"."id" = "registrations"."camp_id"').having('max_students = count("registrations"."id")').group(:camp) }
  scope :empty, -> { joins('left outer join "registrations" on "camps"."id" = "registrations"."camp_id"').having('count("registrations"."id") = 0 ').group(:camp) }


  # callbacks
  before_update :remove_instructors_from_inactive_camp
  before_destroy :check_if_destroyable
  before_save :check_if_deactiveable
  ...
  def is_full? 
    return true if self.registrations.size == self.max_students
    false
  end
  ...
  def remove_instructors_from_camp
    self.camp_instructors.each{|ci| ci.destroy}
  end

  def check_if_destroyable
    cannot_destroy_object() unless self.registrations.empty? 
    remove_instructors_from_camp()
    return true 
  end

  def check_if_deactiveable
    return true if self.active

    self.update_attribute(:active, true) unless self.registrations.empty?
  end
