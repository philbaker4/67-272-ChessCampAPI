...
class CampTest < ActiveSupport::TestCase
  ...
  context "Within context" do
    ...
    should "have a scope that returns all of the full camps" do
      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))
      ...
      validRegist = FactoryBot.create(:registration, student: @appropStudent, camp: @testCamp)

      assert_equal ["Mastering Chess Tactics"], Camp.full.map { |c| c.name  }
    
    end

    should "have a scope that returns all of the empty camps" do
      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))

      assert_equal ["Mastering Chess Tactics"], Camp.empty.map { |c| c.name  }
    
    end

    should "have an 'is_full?' method to determine if camp is full" do 
      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))
      ...
      validRegist = FactoryBot.create(:registration, student: @appropStudent, camp: @testCamp)
      assert @testCamp.is_full?

      @testCamp2 = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,8,23), end_date: Date.new(2018,8,27))
      deny @testCamp2.is_full?
    end 


    should "have an 'enrollment' method to determine how many enrollments there are in the camp" do 
      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))
      ...
      validRegist = FactoryBot.create(:registration, student: @appropStudent, camp: @testCamp)
      assert_equal 1, @testCamp.enrollment
    end 
    ...
    should "only make camp inactive if there are no registrations" do 
      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))

      assert @testCamp.active
      @testCamp.update_attribute(:active, false)
      deny @testCamp.active

      @testCamp.update_attribute(:active, true)
      assert @testCamp.active
    ...
    should "only destroy if there are no registrations" do 
      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))

      assert_equal 5, Camp.all.size
      @testCamp.destroy
      assert_equal 4, Camp.all.size
    ...
    should "remove camp instructors before camp is destroyed" do 

      create_instructors

      @testCamp = FactoryBot.create(:camp, curriculum: @tactics, location: @north, max_students: 1, start_date: Date.new(2018,7,23), end_date: Date.new(2018,7,27))

      @mark_tc = FactoryBot.create(:camp_instructor, instructor: @mark, camp: @testCamp)
      assert_equal 1, CampInstructor.where("camp_id = ? ", @testCamp.id).size

      assert_equal 5, Camp.all.size
      @testCamp.destroy
      assert_equal 4, Camp.all.size

      assert_equal 0, CampInstructor.where("camp_id = ? ", @testCamp.id).size
